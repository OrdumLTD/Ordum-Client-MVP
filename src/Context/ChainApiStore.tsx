...
export const ChainApiContextProvider = ({children}:Props) =>{
    ...
    const [registry, setRegistry] = useState<OnChainRegistry>(defaultChainApiState.phalaRegistry);
    ...
    const fetchPoc5Api = async() =>{
        ...
        let chain_api = await ApiPromise.create(options({
    ...
    return (
        <ChainApiContext.Provider value={{
            fetchChainApi,api:chainApi,poc5,
            phalaRegistry: registry,
            fetchPoc5Api,crustApi,fetchCrustApi
            }}>
            {children}
