...
export const PreimageAndReferendum = async (
  ...
  if (signer && account && amount && beneficiary && chainAPI) {
    ...
    console.log("Amount KSM " + amountKsm.Ksm + "Beneficiary " + beneficiary);
...
export const submitProposal = async (
  ...
  if (account && chainAPI && preimage_data && signer && amount && date) {
    ...
    const tx_submit_hash = chainAPI?.tx.referenda.submit(
      { Origins: Track },
      { Inline: preimage_data },
      { At: blockNumber },
    );

    await tx_submit_hash
