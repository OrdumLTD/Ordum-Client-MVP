...
const getLatestProposalId = async (
  ...
  if (certificate) {
    const data = await contract.query.fetchLatestProposalId(account.address, {
      cert: certificate,
    });
...
const getProposal = async (
  ...
  if (certificate) {
    const data = await contract.query.fetchProposal(
      account.address,
      { cert: certificate },
      id,
    );
    ...
    // Sign the new Certificate and it will update the cache
    const certificate = await onSignCertificate(api, signer, account);
    const data = await contract.query.fetchMilestone(
      account.address,
      { cert: certificate },
      id,
    );
