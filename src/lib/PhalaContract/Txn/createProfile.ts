...
export const createTeamApplicantProfile = async(
    ...
    if(secret.output.toJSON().valueOf()["ok"]['err']){
        ...
        txnData.signAndSend(account.address,{signer},async({isInBlock,events,isFinalized})=>{
            if(isInBlock){
                ...
                // Set the passcode
                await setPasscode(passcodeStatus,account,signer,certificate,contract,api,name);   
