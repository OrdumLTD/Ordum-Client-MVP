...
export const setPasscode = async (
  ...
  const randomData = await contract.query.getRandom(account.address, {
    cert: certificate,
  });
  console.log("Random \n");
  ...
  // Call data
  const txn = contract.tx.setPasscode(options, random, name);
  ...
  txn.signAndSend(account.address, { signer }, ({ isFinalized, events }) => {
    ...
    events.forEach(async ({ event: { method, section } }) => {
      if (method === "ExtrinsicSuccess") {
        ...
        // Waiting function
        const secretValue = await getPasscode(
          contract,
          api,
          signer,
          account,
...
export const getPasscode = async (
  ...
  if (Certificate) {
    const data = await contract.query.getPasscode(account.address, {
      cert: Certificate,
    });
